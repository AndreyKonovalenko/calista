...
import { useChecklistItemActions, useSortedChecklistItemsByChecklistId, useChecklistItems, useChecklistItemsCalclulatedPos } from '../../../services/checklist-item-store';
...
const ChecklistItemDndContainer = memo(
  function ChecklistItemDndContainer(props: { children: React.ReactNode, _id: string, checklistId: string }) {
    ...
    const reNumChecklistItemInChecklist = useReNumChecklistItemsPosInChecklist();
    const {moveChecklistIetem, setSetChecklistItemCalculatedPos} = useChecklistItemActions();
    const updateChecklistItemQuery = useUpdateChecklistItem()
    const sortedChecklistItemsByChecklistId = useSortedChecklistItemsByChecklistId(checklistId);
  ...
  const handleUpdateChecklistItemPos = (
    ...
    >({accept:['checklistItem'],
      hover({_id: draggedId}, monitor) {
        if(!ref.current || draggedId === _id || !checklistItems || !sortedChecklistItemsByChecklistId) {
          return
        }     
        ...
        setSetChecklistItemCalculatedPos(newPos);
        if (newPos !== -1) {
  ...
  const [{isDragging}, connectDrag] = useDrag<TDraggableElement & {checklistId: string}, unknown, { isDragging: boolean}>({type: 'checklistItem', 
    item:{_id, checklistId},
    end({_id: draggedId}, monitor) {
      if (monitor.didDrop()) {
        const dropResult:TDropChecklistItemResult = monitor.getDropResult();
        if (dropResult && dropResult.dropped) {
          ...
          if(dropResult.checklistItemCalculatedPos && dropResult.checklistItemCalculatedPos > 0) {
            handleUpdateChecklistItemPos(draggedId, dropResult.checklistItemCalculatedPos, dropResult.checklistId)
          }
          setSetChecklistItemCalculatedPos(null)
        }
      }
    },
    collect: monitor => ({
  ...
  connectDrop(ref);
    ...
    return <Box sx={dragStyle} ref={ref}>
      {isOver && !isDragging ? (<Box sx={styles.previewStyle}>{children}</Box>): children}
    </Box>;
  },
);
