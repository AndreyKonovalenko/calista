...
const useUIStore = create<IUIState>()(
  devtools(
    set => ({
      ...
      actions: {
        setNewItemAdded: (condition: boolean) =>
          set({ isNewItemAdded: condition }, undefined, 'setNewItemAdded'),
      },
    }),
    {
      name: 'uiStore',
...
export const useIsNewItemAdded = () =>
  useUIStore(state => state.isNewItemAdded);
